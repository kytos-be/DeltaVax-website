...
library(tidyr)
library(scales)
col_KYTOS <- c("#ffa401","#CB0001", "#333333", "#406c89", "#dc6f6f", "#7cd68c")
...
data_bac <- data.frame(
  ...
  Delta_Ras3 = c(723573, 1524507, 749253, 802853, 1242587),
...
ggplot(data_bac, aes(x = Date, y = Bacterial_load, color = System, group = System)) +
  geom_line(linewidth = 1.2, lineend = "round") +
  geom_point(size = 2.8) +
  labs(
    title = "Bacterial load",
    x = NULL,
    y = "Bacterial load (cell/ml)",
    color = NULL
  ) +
  scale_y_continuous(limits = c(0, 1800000), breaks = seq(0, 1800000, 200000)) +
  scale_color_manual(values = col_KYTOS) +
...
data_pro <- data.frame(
  ...
  Delta_Ras3 = c(59.21, 37.31, 49.45, 43.17, 57.4),
...
ggplot(data_pro, aes(x = Date, y = Productivity, color = System, group = System)) +
  geom_line(linewidth = 1.2, lineend = "round") +
  geom_point(size = 2.8) +
  labs(
    title = "Productivity",
...
data_cell <- data.frame(
  ...
  Delta_Ras3 = c(59.43, 39.58, 45.99, 57.17, 39.38),
...
ggplot(data_cell, aes(x = Date, y = Cell_viability, color = System, group = System)) +
  geom_line(linewidth = 1.2, lineend = "round") +
  geom_point(size = 2.8) +
  labs(
...
data_div <- data.frame(
  ...
  Delta_Ras3 = c(3586, 2852, 3586, 3352, 3359),
...
ggplot(data_div, aes(x = Date, y = Diversity, color = System, group = System)) +
  geom_line(linewidth = 1.2, lineend = "round") +
  geom_point(size = 2.8) +
  labs(
    title = "Diversity",
...
ggplot(data_feed, aes(x = factor(Date), y = Gram, fill = Feed)) +
  geom_col(
    width = 0.8,
    color = "white",
    linewidth = 0.3
  ...
  scale_fill_manual(values = col_KYTOS) +
  theme_minimal(base_size = 13) +
  theme(
    plot.title = element_text(face = "bold", size = 16, hjust = 0),
    axis.title.y = element_text(face = "bold", margin = margin(r = 10)),
...
ggplot(data_feed2, aes(x = factor(Date), y = Gram, fill = Feed)) +
  geom_col(
    width = 0.8,
    color = "white",
    linewidth = 0.3
...
ggplot(data_feed3, aes(x = factor(Date), y = Gram, fill = Feed)) +
  geom_col(
    width = 0.8,
    color = "white",
    linewidth = 0.3
...
ggplot(data_feed4, aes(x = factor(Date), y = Gram, fill = Feed)) +
  geom_col(
    width = 0.8,
    color = "white",
    linewidth = 0.3
...
The graphs show fluctuations with an increase in Bacterial Load in the DeltaRAS systems (except Mini RAS), while Diversity also tends to increase. This pattern indicates that the microbial community is responding to the introduction of a new substrate source. According to Rurangwa and Verdegem (2015), changes in feed type can modify the composition of dissolved organic compounds and metabolic by products in aquaculture systems, thereby influencing the structure of microbial communities. Allison and Martiny (2008) further suggested that when a new substrate source emerges or the nutritional spectrum changes, microbial communities tend to increase their functional diversity through niche differentiation.
...
data_compare <- data.frame(
  ...
  SGR = c(17.44, 18.00),
  Survival_Rate = c(7.89,	17.93)
)

data_long <- data_compare %>%
  pivot_longer(
    cols = c(Average_Weight, SGR, Survival_Rate),
    names_to = "Metric",
    values_to = "Value"
  ) %>%
  mutate(
...
ggplot(data_long, aes(x = System, y = Value, fill = System)) +
  geom_col(width = 0.6, color = "black", linewidth = 0.4, show.legend = FALSE) +
  geom_text(aes(label = round(Value, 2)), vjust = -0.4, size = 4) +
  facet_wrap(~Metric, scales = "free_y") +
  scale_fill_manual(values = col_KYTOS[3:4]) +
  labs(
    # title = "Comparison of Delta and Mini",
    x = NULL,
    y = NULL
  ) +
  theme_minimal(base_size = 13) +
